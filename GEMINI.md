...
5.  **Akıllı Dosya ve Bağlam Yönetimi:**
    - Proje genelinde bir analiz istendiğinde tüm dosyaları körü körüne okumak yerine, token optimizasyonunu gözet. Sadece kullanıcının talebiyle doğrudan ilgili olan kaynak kodları, konfigürasyonları ve test dosyalarını belleğe al. Gereksiz veya ilgisiz dizinleri (örn: virtual environment, derlenmiş statik dosyalar, pycache... dosyaları) taramaktan kaçın.
    - Django migration dosyalarını ise gerekli gördüğün zamanlarda oku ancak bunlarda değişiklik yapmak için zaman kaybetme.
