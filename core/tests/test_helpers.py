...
class GetClientIPTest(TestCase):
    ...
    def test_empty_xff_in_trusted_proxies(self):
        request = self.factory.get("/")
        request.META["REMOTE_ADDR"] = "10.0.0.5"
    ...
    def test_whitespace_xff_in_trusted_proxies(self):
        request = self.factory.get("/")
        request.META["REMOTE_ADDR"] = "10.0.0.5"
        request.META["HTTP_X_FORWARDED_FOR"] = "   "
    ...
    def test_irregular_xff_spacing(self):
        request = self.factory.get("/")
        request.META["REMOTE_ADDR"] = "10.0.0.5"
        request.META["HTTP_X_FORWARDED_FOR"] = " 192.168.1.100 , 198.51.100.1 "

        with override_settings(TRUSTED_PROXY_NETS=[ipaddress.ip_network("10.0.0.0/8")]):
            self.assertEqual(get_client_ip(request), "192.168.1.100")

    def test_with_mocked_request_object(self):
        """Test with a simple mock object that only has a META dict."""
        ...
        # Test without XFF
        request = MockRequest({"REMOTE_ADDR": "192.168.1.10"})
        self.assertEqual(get_client_ip(request), "192.168.1.10")

        # Test with XFF and trusted proxy
        request_xff = MockRequest(
            {
                "REMOTE_ADDR": "10.0.0.5",
                "HTTP_X_FORWARDED_FOR": "203.0.113.195, 198.51.100.1",
            }
        )
        with override_settings(TRUSTED_PROXY_NETS=[ipaddress.ip_network("10.0.0.0/8")]):
            self.assertEqual(get_client_ip(request_xff), "203.0.113.195")
