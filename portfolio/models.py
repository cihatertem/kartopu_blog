...
class Portfolio(UUIDModelMixin, TimeStampedModelMixin):
    ...
    def _get_filtered_transactions(
        ...
        if not hasattr(self, "_all_transactions_cache"):
            ...
            tx_list = list(all_txs)
            tx_list.sort(key=lambda tx: (tx.trade_date, tx.created_at))
            self._ensure_transaction_assets_loaded(tx_list)
            self._all_transactions_cache = tx_list

        filtered_txs = [
            tx
            for tx in self._all_transactions_cache
            if not price_date or tx.trade_date <= price_date
        ]

        self._filtered_transactions_cache[price_date] = filtered_txs
        return filtered_txs
